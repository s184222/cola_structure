...
class _DeamortizedCOLA_ConstIterator
{
private:
	using Layer = _DeamortizedCOLA_Layer;
public:
	using PointerType = const int64_t*;
	using ReferenceType = const int64_t&;

public:
	_DeamortizedCOLA_ConstIterator(size_t leftFullFlags, size_t rightFullFlags, uint8_t layerCount,
	...
	_DeamortizedCOLA_ConstIterator& operator++()
	{
	...
	_DeamortizedCOLA_ConstIterator operator++(int)
	{
		_DeamortizedCOLA_ConstIterator itr = *this;
		++(*this);
		return itr;
	}
	...
	bool operator==(const _DeamortizedCOLA_ConstIterator& other) const
	{
...
private:
	using Layer = _DeamortizedCOLA_Layer;
public:
	using ConstIterator = _DeamortizedCOLA_ConstIterator;
